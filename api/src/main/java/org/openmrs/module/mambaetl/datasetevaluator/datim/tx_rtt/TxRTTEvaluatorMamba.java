package org.openmrs.module.mambaetl.datasetevaluator.datim.tx_rtt;
...
@Handler(supports = { TxRTTDataSetDefinitionMamba.class })
public class TxRTTEvaluatorMamba implements DataSetEvaluator {
	
	private static final Log log = LogFactory.getLog(TxRTTEvaluatorMamba.class);
	...
	public DataSet evaluate(DataSetDefinition dataSetDefinition, EvaluationContext evalContext)
		...
		TxRTTDataSetDefinitionMamba dataSetDefinitionMamba = (TxRTTDataSetDefinitionMamba) dataSetDefinition;
	...
	private List<ProcedureCall> createProcedureCalls(TxRTTDataSetDefinitionMamba dataSetDefinitionMamba) {
		...
		return Collections.singletonList(
                new ProcedureCall("{call sp_dim_tx_rtt_datim_query(?,?,?,?)}", statement -> {
                    statement.setDate(1, startDate);
                    statement.setDate(2, endDate);
                    statement.setInt(3, 0);
	                    statement.setString(4, dataSetDefinitionMamba.getTxRTTAggregationTypes().getSqlValue());
                })
        );
	}
}
