package org.openmrs.module.mambaetl.datasetevaluator.datim.prep_ct;
...
@Handler(supports = { PrEPCTDataSetDefinitionMamba.class })
public class PrEPCTEvaluatorMamba implements DataSetEvaluator {
	
	private static final Log log = LogFactory.getLog(PrEPCTEvaluatorMamba.class);
	...
	public DataSet evaluate(DataSetDefinition dataSetDefinition, EvaluationContext evalContext)
		...
		PrEPCTDataSetDefinitionMamba dataSetDefinitionMamba = (PrEPCTDataSetDefinitionMamba) dataSetDefinition;
	...
	private List<ProcedureCall> createProcedureCalls(PrEPCTDataSetDefinitionMamba dataSetDefinitionMamba) {
		...
		return Collections.singletonList(
                new ProcedureCall("{call sp_dim_prep_ct_datim_query(?,?,?,?)}", statement -> {
                    statement.setDate(1, startDate);
                    statement.setDate(2, endDate);
                    statement.setInt(3, 0);
	                    statement.setString(4, dataSetDefinitionMamba.getPrEPCTggregationTypes().getSqlValue());
                })
        );
	}
}
