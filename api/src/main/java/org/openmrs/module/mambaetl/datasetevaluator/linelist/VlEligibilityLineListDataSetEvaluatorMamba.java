...
@Handler(supports = { VlEligibilityLineListDataSetDefinitionMamba.class })
public class VlEligibilityLineListDataSetEvaluatorMamba implements DataSetEvaluator {
	
	@Override
    public DataSet evaluate(DataSetDefinition dataSetDefinition, EvaluationContext evalContext)
            throws EvaluationException {
        VlEligibilityLineListDataSetDefinitionMamba vlEligibilityLineListDataSetDefinitionMamba = (VlEligibilityLineListDataSetDefinitionMamba) dataSetDefinition;
        ...
        validationHelper.validateDates(vlEligibilityLineListDataSetDefinitionMamba, data);
        // Get ResultSet from the database
        try (Connection connection = getDataSource().getConnection();
                CallableStatement statement = connection
                        .prepareCall("{call sp_fact_vl_eligibility_query(?,?)}")) {
            statement.setDate(1, new java.sql.Date(
                    vlEligibilityLineListDataSetDefinitionMamba.getStartDate().getTime()));
            statement.setDate(2, new java.sql.Date(
                    vlEligibilityLineListDataSetDefinitionMamba.getEndDate().getTime()));
