package org.openmrs.module.mambaetl.datasetevaluator.datim.cxca;
...
@Handler(supports = { CXCASCRNDataSetDefinitionMamba.class })
public class CXCASCRNEvaluatorMamba implements DataSetEvaluator {
	
	private static final Log log = LogFactory.getLog(CXCASCRNEvaluatorMamba.class);
	...
	public DataSet evaluate(DataSetDefinition dataSetDefinition, EvaluationContext evalContext)
		...
		CXCASCRNDataSetDefinitionMamba dataSetDefinitionMamba = (CXCASCRNDataSetDefinitionMamba) dataSetDefinition;
	...
	private List<ProcedureCall> createProcedureCalls(CXCASCRNDataSetDefinitionMamba dataSetDefinitionMamba) {
		...
		return Collections.singletonList(
                new ProcedureCall("{call sp_dim_cxca_scrn_datim_query(?,?,?,?)}", statement -> {
                    statement.setDate(1, startDate);
                    statement.setDate(2, endDate);
                    statement.setInt(3, 0);
	                    statement.setString(4, dataSetDefinitionMamba.getCxcatxAggregationTypes().getSqlValue());
                })
        );
	}
}
