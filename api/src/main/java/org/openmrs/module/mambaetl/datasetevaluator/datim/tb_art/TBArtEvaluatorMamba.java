package org.openmrs.module.mambaetl.datasetevaluator.datim.tb_art;
...
@Handler(supports = { TBArtDataSetDefinitionMamba.class })
public class TBArtEvaluatorMamba implements DataSetEvaluator {
	
	private static final Log log = LogFactory.getLog(TBArtEvaluatorMamba.class);
	...
	public DataSet evaluate(DataSetDefinition dataSetDefinition, EvaluationContext evalContext)
		...
		TBArtDataSetDefinitionMamba dataSetDefinitionMamba = (TBArtDataSetDefinitionMamba) dataSetDefinition;
	...
	private List<ProcedureCall> createProcedureCalls(TBArtDataSetDefinitionMamba dataSetDefinitionMamba) {
		...
		return Collections.singletonList(
                new ProcedureCall("{call sp_dim_tb_art_datim_query(?,?,?,?)}", statement -> {
                    statement.setDate(1, startDate);
                    statement.setDate(2, endDate);
                    statement.setInt(3, 0);
	                    statement.setString(4, dataSetDefinitionMamba.getTbArtAggregationTypes().getSqlValue());
                })
        );
	}
}
