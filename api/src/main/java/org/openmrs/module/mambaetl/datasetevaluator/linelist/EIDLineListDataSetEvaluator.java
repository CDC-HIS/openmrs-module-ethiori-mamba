...
public class EIDLineListDataSetEvaluator implements DataSetEvaluator {
    ...
    public DataSet evaluate(DataSetDefinition dataSetDefinition, EvaluationContext evalContext)
        ...
        EIDLineListDatasetDefinition eidLineListDatasetDefinition = (EIDLineListDatasetDefinition) dataSetDefinition;
        SimpleDataSet data = new SimpleDataSet(dataSetDefinition, evalContext);
        ResultSetMapper resultSetMapper = new ResultSetMapper();

        ValidateDates(data, eidLineListDatasetDefinition.getStartDate(), eidLineListDatasetDefinition.getEndDate());
	...
	private List<ProcedureCall> createProcedureCalls(EIDLineListDatasetDefinition dataSetDefinitionMamba) {
        ...
        if ("Rapid Antibody".equals(dataSetDefinitionMamba.getReportType())) {
            procedureName = "{call sp_fact_line_list_eid_rapid_antibody_query(?,?)}";
        } else {
            // Default to DNA PCR
            procedureName = "{call sp_fact_line_list_eid_dna_pcr_query(?,?)}";
        }
