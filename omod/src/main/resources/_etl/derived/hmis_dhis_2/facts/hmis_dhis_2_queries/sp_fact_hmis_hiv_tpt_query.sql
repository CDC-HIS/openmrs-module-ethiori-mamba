...
BEGIN
    WITH FollowUp AS (select follow_up.encounter_id,
         ...
         tmp_art_tpt AS (
             SELECT *,
                    ROW_NUMBER() OVER (PARTITION BY client_id ORDER BY tpt_start_date DESC, encounter_id DESC) AS tpt_row_num
             FROM tpt_base
             WHERE tpt_start_date BETWEEN REPORT_START_DATE AND REPORT_END_DATE
         ),

         tmp_art_tpt_cr AS (
             SELECT *,
                    ROW_NUMBER() OVER (PARTITION BY client_id ORDER BY tpt_start_date DESC, encounter_id DESC) AS tpt_cr_row_num
             FROM tpt_base
             WHERE tpt_start_date BETWEEN
                       fn_ethiopian_to_gregorian_calendar(date_add(fn_gregorian_to_ethiopian_calendar(REPORT_START_DATE, 'Y-M-D'), INTERVAL -12 MONTH))
                       AND
                       fn_ethiopian_to_gregorian_calendar(date_add(fn_gregorian_to_ethiopian_calendar(REPORT_END_DATE, 'Y-M-D'), INTERVAL -12 MONTH))
         ),

         tmp_art_tpt_completed_cr AS (
             SELECT *,
                    ROW_NUMBER() OVER (PARTITION BY client_id ORDER BY tpt_completed_date DESC, encounter_id DESC) AS tpt_completed_cr_row_num
             FROM tpt_base
             WHERE tpt_completed_date <= REPORT_END_DATE
         ),

         art_tpt as (select tmp_art_tpt.tpt_start_date,
                            client.sex,
                            tmp_art_tpt.tb_prophylaxis_type,
                            date_of_birth
                     from tmp_art_tpt
                              left join mamba_dim_client client on tmp_art_tpt.client_id = client.client_id
                     where tpt_row_num = 1),
         art_tpt_cr as (select tpt_started.tpt_start_date,
                               tpt_completed.tpt_completed_date,
                               client.sex,
                               tpt_started.tb_prophylaxis_type,
                               date_of_birth,
                               mrn,
                               uan
                        from tmp_art_tpt_cr tpt_started
                                 left join mamba_dim_client client on tpt_started.client_id = client.client_id
                                 left join tmp_art_tpt_completed_cr tpt_completed  on tpt_started.client_id=tpt_completed.client_id and tpt_completed.tpt_completed_cr_row_num=1
                        where tpt_cr_row_num = 1
         )
