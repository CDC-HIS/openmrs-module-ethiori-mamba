...
WITH FollowUp AS (select follow_up.encounter_id,
     ...
     tmp_twelve_months as (SELECT encounter_id,
                                  client_id,
                           ...
                           WHERE follow_up_status IS NOT NULL
                             ...
                             AND DATE_ADD(art_start_date, INTERVAL 1 YEAR) >= REPORT_START_DATE
                             AND DATE_ADD(art_start_date, INTERVAL 1 YEAR) <= REPORT_END_DATE
                             AND follow_up_date <= REPORT_END_DATE

     ),
     twelve_months as (select curr.client_id,
                              curr.FollowupDate,
                              curr.follow_up_status,
                              curr.pregnancy_status,
                              client.date_of_birth,
                              client.sex
                       from tmp_twelve_months as curr
...
WHERE TIMESTAMPDIFF(YEAR,date_of_birth,REPORT_END_DATE) >= 50
  AND sex='Male'
-- 35 >= 50 years, Female - non-pregnant
UNION ALL
SELECT 'HIV_ART_RET_NET.35'  AS S_NO,
