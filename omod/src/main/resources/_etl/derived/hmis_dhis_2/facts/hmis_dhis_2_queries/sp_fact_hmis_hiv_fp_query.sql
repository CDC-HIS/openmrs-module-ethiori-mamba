...
SELECT 'HIV_ART_FP_MET. 4' AS S_NO,
       ...
       COUNT(*) as Value
FROM family_planning
WHERE method_of_family_planning=''
-- Others
UNION ALL
SELECT 'HIV_ART_FP_MET. 5' AS S_NO,
       ...
       COUNT(*) as Value
FROM family_planning
WHERE method_of_family_planning not in ('Oral contraception','Injectable contraceptives','Implantable contraceptive (unspecified type)');
END //
