...
select fn_ethiopian_to_gregorian_calendar('2016-04-20'); -- 2023-12-01 2023-12-30
WITH Cohort_List AS (SELECT client_id
                     FROM mamba_flat_encounter_pmtct_enrollment
                     WHERE date_of_enrollment_or_booking BETWEEN '2023-12-01' AND '2023-12-30'),
     ...
     TI_Patients AS (SELECT DISTINCT PatientId
                     FROM Follow_UP
                     WHERE transferred_in_check_this_for_all_t = 'Yes'
                       and follow_up_date BETWEEN '2023-12-01' AND fn_ethiopian_to_gregorian_calendar(DATE_ADD(
                             fn_gregorian_to_ethiopian_calendar('2023-12-01', 'Y-M-D'), INTERVAL 25 MONTH))),
     ...
     PMTCT_ENROLLMENT AS (SELECT enrollment.client_id                          as PatientId,
                          ...
                          FROM mamba_flat_encounter_pmtct_enrollment enrollment
                                   LEFT JOIN TI_Patients ON TI_Patients.PatientId = enrollment.client_id
                          WHERE date_of_enrollment_or_booking IS NOT NULL
                            and date_of_enrollment_or_booking BETWEEN '2023-12-01' AND '2023-12-30'
     ...
     PatientIntervals AS (SELECT a.PatientId,
                                 ...
                                 CASE
                                     WHEN i.interval_month = 0 THEN a.date_of_enrollment_or_booking
                                     ELSE fn_ethiopian_to_gregorian_calendar(DATE_ADD(
                                             fn_gregorian_to_ethiopian_calendar('2023-12-01', 'Y-M-D'),
                                             INTERVAL i.interval_month MONTH))
                                     END as interval_end_date,
                                 CASE
                                     WHEN i.interval_month = 0 THEN '2023-12-01'
                                     ELSE LAG(fn_ethiopian_to_gregorian_calendar(DATE_ADD(
                                             fn_gregorian_to_ethiopian_calendar('2023-12-01', 'Y-M-D'),
                                             INTERVAL i.interval_month MONTH)), 1,
                                              '2023-12-01')
     ...
     LatestFollowUpInInterval AS (SELECT pi.PatientId,
                                         ...
                                         f.ARTDoseDays,
                                         f.transferred_in_check_this_for_all_t,
                                         MAX(f.transferred_in_check_this_for_all_t) OVER (PARTITION BY pi.PatientId, pi.interval_month) as interval_ti,
                                         f.AdherenceLevel,
                                         f.pregnancy_status,
                                         f.next_visit_date,
                                         f.current_functional_status,
     ...
     CohortDetails AS (SELECT lfu.PatientId,
                              ...
                              CASE
                                  WHEN lfu.treatment_end_date >= lfu.interval_end_date AND lfu.follow_up_status in ('Alive', 'Restart medication') THEN 'Active'
                                  WHEN meaningful.follow_up_status IN
                                       ('Dead', 'Transferred out', 'Stop all', 'Ran away')
                                      THEN meaningful.follow_up_status
                                  ELSE 'Lost to follow-up'
