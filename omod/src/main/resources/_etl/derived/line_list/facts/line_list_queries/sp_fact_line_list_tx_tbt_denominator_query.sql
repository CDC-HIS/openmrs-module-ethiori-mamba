...
BEGIN
    ...
    SELECT ROW_NUMBER() OVER (ORDER BY patient_name)           as `#`,
           ...
           CASE
               WHEN fn_get_ti_status(f_case.client_id, REPORT_START_DATE, REPORT_END_DATE) = 'NTI' THEN 'No'
               ELSE 'YES' END                                  as `TI?`

    FROM latest_follow_up AS f_case
             INNER JOIN latest_tpt on f_case.client_id = latest_tpt.client_id
