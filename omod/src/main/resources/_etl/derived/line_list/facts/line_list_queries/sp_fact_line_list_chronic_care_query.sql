...
BEGIN
    ...
    SELECT ROW_NUMBER() OVER (ORDER BY patient_name)                 as `#`,
           ...
           client.sex                                                as Sex,
           f_case.enrollement_date                                   as `Enrollment Date EC.`,
...
DROP PROCEDURE IF EXISTS sp_fact_line_list_chronic_care_query_v2;

CREATE PROCEDURE sp_fact_line_list_chronic_care_query_v2(IN REPORT_START_DATE DATE,
                                                      IN REPORT_END_DATE DATE,
                                                      IN STATUS TEXT)
BEGIN
WITH FollowUp AS (select follow_up.encounter_id,
