...
DROP PROCEDURE IF EXISTS sp_fact_line_list_positive_tracking_query_v2;

CREATE PROCEDURE sp_fact_line_list_positive_tracking_query_v2(IN REPORT_START_DATE DATE,
                                                           IN REPORT_END_DATE DATE)
BEGIN

         -- LATE ROW LOOKUP ID RESOLUTION FOR POSITIVE TRACKING
         WITH tmp_positive_tracking as (select p_tracking.date_when_final_outcome_was_known   as date_final_outcome_known,
                                          p_tracking.client_id,
                                          p_tracking.date_of_hiv_diagnosis               as date_hiv_confirmed,
                                          p_tracking.entry_point                         as entry_point,
                                          p_tracking.linked_to_hiv_care                  as linked_to_treatment,
        ...
        positive_tracking as ( select * from tmp_positive_tracking where row_num=1),
         ...
         -- LATE ROW LOOKUP FOR FOLLOWUP
         tmp_latest_follow_up_ids AS (SELECT client_id, encounter_id,
                                         ROW_NUMBER() OVER (PARTITION BY client_id ORDER BY follow_up_date_followup_ DESC,
                                             encounter_id DESC) AS row_num
                                  FROM vw_mamba_fact_encounter_follow_up
                                  WHERE follow_up_date_followup_ <= COALESCE
                                                          (REPORT_END_DATE, CURDATE())),

         latest_follow_up_ids AS (select encounter_id
                              from tmp_latest_follow_up_ids
                              where row_num = 1),

         -- HEAVY DATA FETCH PHASE
         latest_follow_up AS (
