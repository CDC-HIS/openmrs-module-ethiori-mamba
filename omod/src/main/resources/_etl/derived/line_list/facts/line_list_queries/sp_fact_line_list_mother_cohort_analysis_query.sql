...
BEGIN
    ...
    WITH Follow_UP AS (SELECT follow_up.client_id                 as PatientId,
         ...
         -- Combines the latest follow-up details and calculates Regimen Line logic once.
         CohortDetails AS (SELECT lfu.PatientId,
                                  ...
                                  -- Pre-calculate Regimen Line Logic here to avoid repetition in final SELECT
                                  CASE
    ...
    SELECT dc.patient_name                                                                AS 'Patient Name',
           ...
           COALESCE(MAX(CASE WHEN co.interval_month = 0 THEN co.AdherenceLevel ELSE NULL END), 0)   AS 'Latest Adherence at Zero Months',
           ...
           COALESCE(MAX(CASE WHEN co.interval_month = 6 THEN co.ARTDoseDays ELSE NULL END), 0)      AS 'Latest Regimen Dose Days at 6 Months',
           MAX(CASE WHEN co.interval_month = 6 THEN co.follow_up_status ELSE NULL END)    AS 'Latest Follow-up Status at 6 Months',
           COALESCE(MAX(CASE WHEN co.interval_month = 6 THEN co.AdherenceLevel ELSE NULL END), 0)   AS 'Latest Adherence at 6 Months',
           ...
           COALESCE(MAX(CASE WHEN co.interval_month = 12 THEN co.ARTDoseDays ELSE NULL END), 0)     AS 'Latest Regimen Dose Days at 12 Months',
           MAX(CASE WHEN co.interval_month = 12 THEN co.follow_up_status ELSE NULL END)   AS 'Latest Follow-up Status at 12 Months',
           COALESCE(MAX(CASE WHEN co.interval_month = 12 THEN co.AdherenceLevel ELSE NULL END), 0)  AS 'Latest Adherence at 12 Months',
           ...
           COALESCE(MAX(CASE WHEN co.interval_month = 24 THEN co.ARTDoseDays ELSE NULL END), 0)     AS 'Latest Regimen Dose Days at 24 Months',
           MAX(CASE WHEN co.interval_month = 24 THEN co.follow_up_status ELSE NULL END)   AS 'Latest Follow-up Status at 24 Months',
           COALESCE(MAX(CASE WHEN co.interval_month = 24 THEN co.AdherenceLevel ELSE NULL END), 0)  AS 'Latest Adherence at 24 Months',
