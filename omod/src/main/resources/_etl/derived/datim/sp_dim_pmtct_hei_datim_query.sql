...
BEGIN
    ...
    SET base_query = CONCAT(
        ...
        '    SELECT hiv_test.client_id, hiv_test_date, date_of_birth, hiv_test_result, ',
        ...
        '    FROM tmp_hei_test ',
        '    WHERE row_num = 1 AND age_in_months <= 12 ',
        '), ',
        'categories AS ( ',
    ...
    ELSE
        -- DEBUG mode
        SET final_query = CONCAT(base_query,
            'SELECT client_id AS Client, result_category AS Result, age_category AS Age_Category, age_in_months AS Age_Months FROM hei_test'
        );
    END IF;
