...
BEGIN
    ...
    SET base_query = CONCAT(
        ...
        '    SELECT hiv_test.client_id, hiv_test_date, date_of_birth, hiv_test_result, test_round, ',
    ...
    IF REPORT_TYPE = 'NUMERATOR' THEN
        SET final_query = CONCAT(base_query,
            'SELECT COALESCE(COUNT(DISTINCT client_id), 0) AS Numerator FROM hei_test'
        );
    ...
    ELSEIF REPORT_TYPE = 'TOTAL' THEN
        SET final_query = CONCAT(base_query,
            'SELECT ',
            '    (SELECT COALESCE(COUNT(DISTINCT client_id), 0) FROM hei_test) AS Numerator, ',
    ...
    ELSE
        -- DEBUG mode
        SET final_query = CONCAT(base_query,
            'SELECT client_id AS Client, test_category AS Test_Category, age_category AS Age_Category, age_in_months AS Age_Months FROM hei_test'
        );
    END IF;
