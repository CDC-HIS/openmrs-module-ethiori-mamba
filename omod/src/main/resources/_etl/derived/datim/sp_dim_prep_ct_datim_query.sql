...
BEGIN
    ...
    SET prep_query = 'WITH PreExposure AS (select screening.client_id,
     ...
     tmp_latest_follow_up as (select prep_dose_end_date,
                                     ...
                                     (SELECT fine_age_group
                                      ...
                                      where TIMESTAMPDIFF(YEAR, date_of_birth, follow_up_date_followup_) = age) as fine_age_group,
                                     (SELECT coarse_age_group
                                      from mamba_dim_agegroup
                                      where TIMESTAMPDIFF(YEAR, date_of_birth, follow_up_date_followup_) = age) as coarse_age_group,
    ...
    ELSEIF REPORT_TYPE='TEST_RESULT' THEN
        SET group_query= 'select ''Positive'' as `Name`, COUNT(*) from prep where hiv_test_result=''Positive''
UNION ALL
select ''Negative'' as `Name`, COUNT(*) from prep where hiv_test_result=''Negative result''
UNION ALL
select ''Other'' as `Name`, COUNT(*) from prep where hiv_test_result NOT IN (''Negative result'',''Positive'')';
